...
public class urlclass {
    ...
    public class Pics
    {
        ...
        public PicForGame generatePic()
        {
            ...
            //перемешиваем массив
            pic.Answers4 = pic.Answers4.OrderBy(x=>rnd.Next());
    ...
    public class Handler
    {
        ...
        public bool regimUgadaika = false;
        public string RightAnswer = "";
        public int ugadal = 0;
        public int neugadal = 0;
        public async Task<IAliceResponse> FunctionHandler(string requestJ)
        {
            ...
            else
            {
                ...
                else if (regimUgadaika)
                {
                    if (request.Request.Command.ToLower().Contains(RightAnswer))
                    {
                        ugadal++;
                        AliceResponseCard resp = new AliceResponseCard();
                        resp.Response.Text = "ты угадал, молодец! угадал раз="+ ugadal;
                    ...
                    else
                    {
                        neugadal++;
                        AliceResponseCard resp = new AliceResponseCard();
                        resp.Response.Text = "ты НЕ угадал, правильный ответ был "+ RightAnswer+ " НЕ угадал " + neugadal + " " + "раз";
                        resp.Response.Tts = "ты НЕ угадал, правильный ответ был " + RightAnswer;
                        resp.Response.card = new Card();
                        return resp;
                    }
                    regimUgadaika = false;
                ...
                else if (request.Request.Command.ToLower().Contains("угадайка"))
                {
                    try
                    {
                        ...
                        Random rnd = new Random();
                        pg.Answers4 = pg.Answers4.OrderBy(x => rnd.Next());
